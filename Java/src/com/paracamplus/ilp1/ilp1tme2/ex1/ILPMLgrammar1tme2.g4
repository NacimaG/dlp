...
@header {
    //com.paracamplus.ilp1.ilp1tme2.ex1
    package antlr4;
}
...
prog returns [com.paracamplus.ilp1.interfaces.IASTprogram node] 
    : (fundef=globalfundef)* (exprs+=expr ';'?) * EOF
    ;
    
globalfundef:
    'function' nom = IDENT  '(' param+=IDENT? (',' param+=IDENT)* ')' operations=expr # FunDef
   ;
