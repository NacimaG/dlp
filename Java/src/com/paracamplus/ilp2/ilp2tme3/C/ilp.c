...
//****************************************************************AJOUT**************
ILP_Object ILP_sinus(ILP_Object d) {
	if (ILP_isInteger(d)){
        ILP_Object result = ILP_AllocateFloat();
		result->_content.asFloat = sin(d->_content.asInteger);
		return result;
	}
	if (ILP_isFloat(d)){
	        ILP_Object result = ILP_AllocateFloat();
			result -> _content.asFloat = sin(d->_content.asFloat);
			return result;
		}
	return ILP_domain_error("error sinus", d);

}


ILP_Object ILP_makeVector(ILP_Object t,ILP_Object v) {
	if (!ILP_isInteger(t)){
		return ILP_domain_error("taille not integer", t);
	}
	int size = t->_content.asInteger;
	ILP_Object result = ILP_AllocateVector(size);
	result->_content.asVector.size=size;

	for (int i = 0; i < size; i++)
		result->_content.asVector.content[i] = v;
	return result;

}


ILP_Object ILP_vectorLength(ILP_Object v) {
	if (!ILP_isVector(v)){
		return ILP_domain_error("arg not vector", v);
	}
	ILP_Object result = ILP_AllocateInteger();
	result->_content.asInteger = v->_content.asVector.size;
	return result;

}

ILP_Object ILP_vectorGet(ILP_Object v,ILP_Object ind) {
	if (!ILP_isVector(v)){
		return ILP_domain_error("arg not vector", v);

	}
	if(!ILP_isInteger(ind)){
		return ILP_domain_error("indice doit etre un entier",ind);
	}

	int i= ind->_content.asInteger;
	int taille= v->_content.asVector.size;
	if(taille<=i){
		return ILP_domain_error("indice doit etre inferieure a la capacite du vector",v);
	}

	ILP_Object o = v->_content.asVector.content[i];
	return o;
